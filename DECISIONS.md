...
- **2026-07-23 — Production deployed early (before milestone 3):** hosted Supabase project `fvsupccynnhqeywpuukg` in `ap-southeast-1` (Singapore, closest region to PH); Vercel project `alonapp` with GitHub push-to-deploy from `judegamboa/alonapp` main; live at https://alonapp.vercel.app (alonapp.com DNS deferred). All 5 migrations pushed; auth config (custom access token hook) synced via `supabase config push`. DB password lives with the owner only (resettable in the dashboard), service-role key only in Vercel production env.
